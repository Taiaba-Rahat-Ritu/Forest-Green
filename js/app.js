  ...
  for(const product of allProducts) { 
   console.log(product);
    ...
    <img class="product-image" src=${image}></img>
      ...
      <p>Total rating given <b>${product.rating.count}</b> people</p>
      <h4>Average rating <b>${product.rating.rate}</b> </h4>

      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success addToCartbtn">add to cart</button>
      <button id="details-btn" class="btn btn-danger detailsBtn">Details</button></div>
...
// set innerText for tax function
...
const updateTotalValue=()=>{
  ...
  const totalPriceValueText = totalPrice.innerText;
  const totalPriceValueTextValue=parseFloat(totalPriceValueText);
  // console.log(totalPriceValueTextValue);
  // delivery charge 
  const totalDeliveryCharge=document.getElementById('delivery-charge');
  const totalDeliveryChargeText = totalDeliveryCharge.innerText;
  const totalDeliveryChargeTextValue=parseFloat(totalDeliveryChargeText);
  // console.log(totalDeliveryChargeTextValue);

  // total tax 
  const totalTax=document.getElementById('total-tax');
  const totalTaxText = totalTax.innerText;
  const totalTaxValue=parseFloat(totalTaxText);
  // console.log(totalDeliveryChargeTextValue);
  ...
  // const inTotalText=inTotal.innerHTML
  // const inTotalTextValue= parseFloat(inTotalText);
  inTotal.innerHTML=(totalPriceValueTextValue+totalDeliveryChargeTextValue+totalTaxValue).toFixed(2);
